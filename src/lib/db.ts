...
if (!(globalThis as any).__products) {
  (globalThis as any).__products = [...initialProducts];
  (globalThis as any).__orders = [];
  (globalThis as any).__users = [];
  (globalThis as any).__adminInitialized = false;
}

function store<T>(key: string, fallback: T): { get(): T; set(val: T): void } {
  return {
    get(): T {
      if ((globalThis as any)[key] === undefined) {
        (globalThis as any)[key] = fallback;
      }
...
export const db = {
  users: store<StoreUser[]>('__users', []),
  products: store<Product[]>('__products', initialProducts),
  orders: store<StoreOrder[]>('__orders', []),

  get adminInitialized(): boolean {
    return (globalThis as any).__adminInitialized || false;
  },
  set adminInitialized(val: boolean) {
    (globalThis as any).__adminInitialized = val;
  },
};
