...
describe('validateOrderItems', () => {
  it('debería_aceptar_items_con_precios_del_catálogo_y_calcular_el_total', async () => {
  ...
  it('debería_rechazar_precio_manipulado', async () => {
    const result = await validateOrderItems([
  ...
  it('debería_rechazar_precio_manipulado_con_campo_price_de_orders', async () => {
    const result = await validateOrderItems([
  ...
  it('debería_respetar_el_precio_de_oferta_del_catálogo', async () => {
    // funda-floral-rose tiene price 299000 y no salePrice; se cobra el base.
    const normal = await validateOrderItems([
      { productId: 'funda-floral-rose', quantity: 1, unit_price: 299000 },
    ]);
    expect(normal.ok).toBe(true);
  });

  it('debería_rechazar_producto_inexistente', async () => {
    const result = await validateOrderItems([
  ...
  it('debería_rechazar_producto_inactivo', async () => {
    // Desactivar un producto existente y verificar que no se puede comprar
    await getProductRepo().update('termo-vidrio-350ml', { active: false });
  ...
  it('debería_rechazar_cantidad_inválida_o_excesiva', async () => {
    const zero = await validateOrderItems([
      { productId: 'funda-silicone-clear', quantity: 0, unit_price: 249000 },
    ]);
    expect(zero.ok).toBe(false);
    expect(zero.errors).toContain('Cantidad inválida');

    const excesiva = await validateOrderItems([
  ...
  it('debería_rechazar_sin_precio', async () => {
    const result = await validateOrderItems([
  ...
  it('debería_rechazar_lista_vacía', async () => {
    const result = await validateOrderItems([]);
    expect(result.ok).toBe(false);
  });
});
